  ...
  flows.forEach((f, idx) => {
    ...
    btn.addEventListener("click", async () => {
      ...
      if (f.nodes && f.nodes.length > 0) {
        const firstNodeId = f.nodes[0].id;
        await updateTimeline(firstNodeId);
        updateEventsTable(f, firstNodeId);
      } else {
        console.warn("No nodes in this flow:", f.title);
      }
    });
...
async function updateEventsTable(flow, nodeId) {
  ...
  try {
    // 노드 기준 이벤트 목록 가져오기
    const res = await fetch(`/api/events_by_node/${nodeId}`);
    const events = await res.json();

    if (events.length === 0) {
      tableBody.innerHTML = `<tr><td colspan="6" class="text-center text-gray-400 py-2">해당 노드의 이벤트가 없습니다.</td></tr>`;
      return;
    }
    ...
    for (const ev of events) {
      ...
      row.innerHTML = `
        <td class="px-2 py-1 text-xs text-gray-600">${ev.timestamp}</td>
        <td class="px-2 py-1 text-xs">${ev.src_ip} → ${ev.dst_ip}</td>
        <td class="px-2 py-1 text-xs">${ev.tatic || "-"}</td>
        <td class="px-2 py-1 text-xs">${ev.technique || "-"}</td>
        <td class="px-2 py-1 text-xs">${ev.severity || "-"}</td>
      `;
      tableBody.appendChild(row);
    }
  } catch (err) {
    console.error("이벤트 테이블 업데이트 실패:", err);
    tableBody.innerHTML = `<tr><td colspan="6" class="text-center text-red-500 py-2">이벤트를 불러오는 중 오류 발생</td></tr>`;
  }
}
